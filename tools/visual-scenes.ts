...
function enemyIntentScene(): VisualScene {
  ...
  // A spine broken by a two-row gap on rows 4-5. The gap is what the plans on
  // this board are made of: the Marksman gets the long straight line its lock
  // depends on, the Sentinel gets a diagonal sightline onto that same target
  // from two rows above it, and the Rifleman is left ground it has to cross.
  for (const y of [2, 3, 6, 7, 8]) setTile(map, 9, y, "wall");
  ...
  const sentinel = makeArchetypeUnit("sentinel", "intent-sentinel", 12, 3);
  const scrapper = makeArchetypeUnit("scrapper", "intent-scrapper", 7, 9);
  ...
  // Vex holds the lane. That is real board state, not decoration: the Sentinel
  // watching the same ground prices a watch it will not walk through and plans
  // to break it instead, which is how a suppression banner reaches this board
  // honestly rather than by being written here.
  performAction(map, vex, "overwatch");
  ...
  // Four banners that all say the same thing review as one banner. The board is
  // arranged so the planner reaches four different conclusions on it; if a
  // planner change collapses them, this scene has stopped doing its job and
  // says so here rather than quietly rendering a duller picture.
  const plans = new Set(
    [marksman, sentinel, scrapper, rifleman].map((enemy) => enemy.intent?.kind),
  );
  if (plans.size < 4) {
    throw new Error(
      `Enemy-intent scene expected four different plans, got ${[...plans].join(", ")}`,
    );
  }
  ...
  return {
    ...
    description:
      "Four hostiles publishing four different plans - a locked-on Marksman, a Sentinel " +
      "about to suppress Vex's overwatch, a Scrapper already engaging, and a Rifleman " +
      "still closing - every banner generated by the production AI planner rather than " +
      "written for the lab.",
    review:
